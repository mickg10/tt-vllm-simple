...
# Read HF_MODEL from env file (used for model-agnostic verify-device)
_HF_MODEL = $(shell grep -m1 '^HF_MODEL=' $(DEVICE_ENV) 2>/dev/null | cut -d= -f2)
...
verify-device:
	...
	@response=$$(curl -sf --max-time 30 http://localhost:$(PORT)/v1/completions \
		...
		-d '{"model":"$(_HF_MODEL)","prompt":"The capital of France is","max_tokens":10,"temperature":0}' 2>&1) || \
	...
	echo "$$response" | python3 -c "$$( printf '%s\n' \
		...
		'    print("Response:", text)' \
		'    assert len(text.split()) > 2, "Output may be garbled"' \
